...
object HuffingPostProposals extends Migration {
  ...
  var huffingPostUser: Seq[User] = _
  ...
  private def createUsers(api: MakeApi) = {
    ...
    sequentially(huffingPostUser) { user =>
  ...
  override def migrate(api: MakeApi): Future[Unit] = {
    ...
    Future {
      operationsProposalsSource.foreach {
        case (operationSlug, proposalsDataSource) =>
          println(s"operation $operationSlug")
          ...
          retryableFuture(
            ...
            case Some(operation) =>
              val proposalsToInsert: Seq[ProposalsDataLine] =
                Source.fromResource(proposalsDataSource).getLines().toSeq.drop(1).flatMap(extractDataLine)

              retryableFuture(api.persistentTagService.findByOperationId(operation.operationId)).flatMap {
                case tagsOfOperation: Seq[Tag] =>
                  sequentially(proposalsToInsert) { proposalsToInsert =>
                    println(s"operation $operationSlug -- Proposal to insert ${proposalsToInsert.content}")
                    val tags: Seq[Tag] = tagsOfOperation.filter(tag =>  proposalsToInsert.tagsLabel.contains(tag.label))
                        ...
                        SearchQuery(
                          filters = Some(
                            SearchFilters(
                              ...
                              operation = Some(OperationSearchFilter(operation.operationId))
                            )
                          )
                        )
                      )
                          ...
                          for {
                            ...
                            idea <- retryableFuture(
                              retrieveOrInsertIdea(
                                name = proposalsToInsert.content,
                                language = proposalsToInsert.language,
                                country = proposalsToInsert.country,
                              ...
                              retryableFuture(
                                api.proposalService.validateProposal(
                                  ...
                                  ValidateProposalRequest(
                                    ...
                                    operation = Some(operation.operationId),
                                    idea = Some(idea.ideaId)
                                  )
                                )
                              )
                          } yield {
                            println(s"tags: ${tagsOfOperation.map(_.label).mkString(",")} for proposal ${proposalsToInsert.content}")
                          }
                        }
                      }
                    Future.successful{}
                ...
                case _ =>
                  Future.failed(new RuntimeException(s"Error When getting operation tags for $operationSlug"))
              }

              Future.successful{}
            case None =>
              Future.failed(new RuntimeException(s"Error When getting operation $operationSlug when importing proposals"))
          }
        case _ =>
          Future.failed(new RuntimeException(s"Error When importingHuffingPost proposals"))
      }
    }
  }

  def extractDataLine(line: String): Option[ProposalsDataLine] = {
    line.drop(1).dropRight(1).split("""";"""") match {
      case Array(email, content, tagsString) =>
        val tagsLabel: Seq[String] = tagsString.split('|').toSeq
  ...
  case class ProposalsDataLine(email: String,
                               content: String,
