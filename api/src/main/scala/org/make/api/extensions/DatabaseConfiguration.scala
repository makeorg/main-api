...
class DatabaseConfiguration(override protected val configuration: Config)
  ...
  ConnectionPool.add(
    'READ,
    new DataSourceConnectionPool(dataSource = readDatasource)
  )

  ConnectionPool.add(
    'WRITE,
    new DataSourceConnectionPool(dataSource = writeDatasource)
  )
  ...
  if (autoCreateSchemas) {
    ...
    Try(createSchema) match {
      ...
      case Failure(e) =>
        logger.error(
          s"Cannot create schema: ${e.getStackTrace.mkString("\n")}"
        )
    }
  }
}
...
object DatabaseConfiguration
    extends ExtensionId[DatabaseConfiguration]
    with ExtensionIdProvider {
  override def createExtension(
    system: ExtendedActorSystem
  ): DatabaseConfiguration =
    new DatabaseConfiguration(
      system.settings.config.getConfig("make-api.database")
    )
