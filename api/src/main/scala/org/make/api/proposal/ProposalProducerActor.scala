...
class ProposalProducerActor extends ProducerActor[ProposalEventWrapper] {
  ...
  override def receive: Receive = {
    ...
    case event: ProposalAddedToOperation     => onEventProposal(event, ProposalAddedToOperation.version)
    case event: ProposalRemovedFromOperation => onEventProposal(event, ProposalRemovedFromOperation.version)
