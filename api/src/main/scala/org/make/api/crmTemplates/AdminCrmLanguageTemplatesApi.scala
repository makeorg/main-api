...
object CrmLanguageTemplates {
  ...
  def apply(language: Language, templates: CrmTemplateKind => CrmLanguageTemplate): CrmLanguageTemplates =
    CrmLanguageTemplates(
      ...
      resendRegistration = templates(CrmTemplateKind.Welcome).template,
      welcome = templates(CrmTemplateKind.ProposalAccepted).template,
      proposalAccepted = templates(CrmTemplateKind.ProposalRefused).template,
      proposalRefused = templates(CrmTemplateKind.ForgottenPassword).template,
      forgottenPassword = templates(CrmTemplateKind.ResendRegistration).template,
