...
object ConcertationEvent {
  def fromConcertationRequest(
    request: ConcertationTrackingRequest,
    applicationName: Option[ApplicationName]
  ): ConcertationEvent =
    ConcertationEvent(eventName = request.eventName, context = request.context, parameters = request.parameters)
}
