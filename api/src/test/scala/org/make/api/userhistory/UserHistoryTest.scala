...
class UserHistoryTest extends MakeUnitTest with ActorSystemTypedComponent {
  override val actorSystemTyped: ActorSystem[_] = UserHistoryTest.system
  ...
  Feature("source value of register event after deserialization") {
    Scenario("I get 'core' as source value with an event without source in context and action date after 2018-09-01") {
      ...
      val registerEvent: LogRegisterCitizenEvent =
        StaminaTestUtils
          .deserializeEventFromJson[LogRegisterCitizenEvent](actorSystemTyped, eventType, registeredEventWithoutContext)
    ...
    Scenario(
      ...
      val registerEvent: LogRegisterCitizenEvent =
        StaminaTestUtils.deserializeEventFromHexa[LogRegisterCitizenEvent](actorSystemTyped, serialized)
