...
class RetryableFutureTest extends MakeUnitTest {
  ...
  Feature("Retry on futures") {
    Scenario("future succeeds") {
      ...
      val result = RetryableFuture.retry(() => Future.successful(defaultValue), alwaysRetry)
    ...
    Scenario("future always fails") {
      val result = RetryableFuture.retry(() => Future.failed(new IllegalStateException("fake")), alwaysRetry)
    ...
    Scenario("future fails once") {
      ...
      whenReady(RetryableFuture.retry(function, alwaysRetry), Timeout(3.seconds)) {
        _ should be(defaultValue)
      }
    }
    ...
    Scenario("retry on registered exceptions") {
      ...
      whenReady(RetryableFuture.retry(function, retryOnIllegalStateException).failed, Timeout(3.seconds)) {
