...
import org.make.core.operation.indexed.OperationOfQuestionElasticsearchFieldNames._

sealed abstract class SortAlgorithm(priority: FieldSort*) extends EnumEntry {

  private val sort = priority.toSeq ++ Seq(
    FieldSort(field = endDate, order = SortOrder.DESC, missing = Some("_first")),
    FieldSort(field = slug, order = SortOrder.ASC, missing = None)
  )
...
object SortAlgorithm extends Enum[SortAlgorithm] {
  ...
  case object Featured extends SortAlgorithm(FieldSort(field = featured, order = SortOrder.DESC, missing = None))
