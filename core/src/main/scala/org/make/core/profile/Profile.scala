...
sealed trait Gender {
  ...
  implicit lazy val genderDecoder: Decoder[Gender] =
    Decoder.decodeString.map(gender => Gender.matchGender(gender).getOrElse(
      throw new IllegalArgumentException(s"$gender is not a Gender")
    ))
}

object Gender extends StrictLogging {
  val genders: Map[String, Gender] = Map(
    Male.shortName -> Male,
    Female.shortName -> Female,
...
object Profile {
  ...
  def parseProfile(dateOfBirth: Option[LocalDate],
                   avatarUrl: Option[String],
                   profession: Option[String],
                   phoneNumber: Option[String],
                   twitterId: Option[String],
