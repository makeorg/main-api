...
import org.make.api.user.DefaultPersistentUserServiceComponent.{FollowedUsers, PersistentUser}
        ...
        withSQL {
          update(PersistentUser)
            .set(
              ...
              column.lastIp -> user.lastIp,
              column.lastConnection -> Option(DateHelper.now()),
              column.avatarUrl -> user.profile.flatMap(_.avatarUrl),
              column.facebookId -> user.profile.flatMap(_.facebookId),
              column.googleId -> user.profile.flatMap(_.googleId),
              column.gender -> user.profile.flatMap(_.gender),
  ...
  implicit object PersistentUser extends PersistentCompanion[PersistentUser, User] with ShortenedNames with Logging {
    ...
    def apply(
      ...
      PersistentUser.apply(
        ...
        lastConnection = resultSet.zonedDateTimeOpt(userResultName.lastConnection),
